...
def get_int(prompt):
    ...
    Note:
    The function will repeatedly ask
    the user until a valid integer is provided.
    """
    while True:
        value = input(prompt)
...
def lst_of_items(lst, name, total_1=0, total_2=0):
    """
    Print items from list of dictionary,
    typically representing warehouse inventories.
...
def ask_for_max(name, total, item_name):
    ...
    This function is executed when user types
    in more amount than the 'total' parameter.
    """
    while True:
...
def ask_for_placing_order(name, total, item_name):
    ...
    Returns:
    None. This function only interacts with
    the user via print statements and input prompts.
    ...
    Various messages are shown based on
    the available stock and desired quantity.
    """
    continue_loop = True
...
def searching_for_item(
    name, total_1=0, total_2=0, days_since_then1=None, days_since_then2=None
):
    """
...
def browse_by_category(name, counter=1, product_counter=None, product_dct=None):
    ...
    Upon selecting a category number, it prints all
    products in that category along with their warehouses.
